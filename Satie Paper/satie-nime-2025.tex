...
\begin{teaserfigure}
  ...
  \caption{A Satie script (left) alongside its corresponding Unity scene (right). Each line in the script maps to a spatialized sound source whose 3D trajectory is visualized in the scene with colored trails and spheres.}
  \Description{Split view showing a Satie script on the left and a Unity 3D scene on the right with colored visual trails tracing sound source trajectories.}
  \label{fig:satie}
\end{teaserfigure}
...
Randomization & Five different bird sounds & \texttt{5 * oneshot gen} \\
              ...
              & Varying pitch and volume & \texttt{pitch 0.5to1.1}, \texttt{volume 0.1to0.5} \\
